...
export class Filters {
    ...
    this.ready = this.storage.get(FILTERS_KEY).then((d: any) => {
      ...
      if (typeof d === 'object') {
        this.data.tags = Array.isArray(d.tags)
          ...
          : Array.isArray(d.blockedTags)
          ? d.blockedTags.map(normTag).filter(Boolean)
          : [];
  ...
  private loadEntities(modern: any, legacyIds: any): BlockedEntity[] {
    ...
    if (Array.isArray(legacyIds)) {
      return legacyIds.map(id => normEntity(id)).filter(Boolean) as BlockedEntity[];
    }
    return [];
  }
  ...
  private addEntity(key: 'authors' | 'series' | 'categories', idOrEntity: any, name?: string): boolean {
    const ent = normEntity(typeof idOrEntity === 'object' && idOrEntity !== null ? idOrEntity : { id: idOrEntity, name: name || '' });
    if (!ent) return false;
